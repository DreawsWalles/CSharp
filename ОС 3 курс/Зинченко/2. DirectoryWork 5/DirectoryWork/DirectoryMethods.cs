...
namespace DirectoryWork
{
    class DirectoryMethods
    {
        ...
        /// <summary>
        /// Функция для возврата всех имен каталогов по заданному пути
        /// </summary>
        /// <param name="path">Заданный путь/param>
        /// <returns>Список имен каталогов</returns>
        public static IEnumerable<string> GetAllDirectoriesNames(string path)
        {
            // Сначала перебираем подкаталоги первого уровня вложенности...
            foreach (string subDir in GetDirectories(path))
            {
                ...
                // вложенного итератора на каждом шаге рекурсии, но затраты на создание
                // такого объекта относительно не велики, а удобство очень даже ощутимо.
                foreach (string subDir2 in GetAllDirectoriesNames(relativePath))
                    yield return subDir2;
            }
        }
