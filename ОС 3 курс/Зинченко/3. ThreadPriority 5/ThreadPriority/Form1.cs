...
namespace ThreadPriority
{
    public partial class Form1 : Form
    {
        ...
        void ShowMaxPrior()
        {
            int maxLen = th.MaxPriority();
            textBoxOrig.Invoke(new Action(() => textBoxResult.AppendText("Максимальный приоритет: " + maxLen.ToString() + "\r\n\r\n")));
            Invoke(new Action(() => buttonMakeTask.Enabled = false));

            foreach (ProcessEntry32 process in th.GetProccessesMaxPriorInRange(maxLen))
            {
