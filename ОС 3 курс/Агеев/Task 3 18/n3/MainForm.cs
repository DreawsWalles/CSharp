        ...
        void GetAllProcesses()
        {
            lock(this)
            {
                ...
                foreach (ProcessEntry32 pe in th.GetProcessList())
                    ProcessTB.Invoke(new Action(() => ProcessTB.AppendText(pe.ToString() + "Количество модулей: " + th.GetModuleList(pe.th32ProcessID).Count().ToString() + Environment.NewLine + Environment.NewLine)));
        ...
        void GetProcWithMods()
        {
            ...
            else
            {
                foreach (ProcessEntry32 pe in result)
                    ResultTB.Invoke(new Action(() => ResultTB.AppendText(pe.ToString() + "Количество модулей: " + th.GetModuleList(pe.th32ProcessID).Count().ToString() + Environment.NewLine + Environment.NewLine)));
            }
