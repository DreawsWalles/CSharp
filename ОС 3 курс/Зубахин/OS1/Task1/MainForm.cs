...
namespace Task1
{
    public partial class main_frm : Form
    {
        ...
        private void btnrun_Click(object sender, EventArgs e)
        {
            ...
            ReaderTB.Clear();
            WriterTB.Clear();
            BufStack Buf = new BufStack(mainTB, int.Parse(buf_size.Text));
       ...
       private void btnWriters_Click(object sender, EventArgs e)
        ...
        {
            MessageBox.Show("1.3.4 Представление буфера - стек \n Средства синхронизации - мониторы \n Создать многопоточное приложение, в котором главный поток в случайные моменты времени порождает либо поток-читатель, который в случайные моменты времени удаляеися данные из буфера, либо поток-писатель, который в случайные моменты времени помещает данные в буфер и сообщает об этом. Читатель и писатель завершаются после совершение заданного кол-ва действий", "Task");
        }
    }
}
