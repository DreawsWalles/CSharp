...
namespace ThreadPriority
{
    ...
    /// </summary>
    [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Auto)]//для последовательного расположения полей класса в памяти
    public struct ProcessEntry32
    {
        const int MAX_PATH = 260;//максимальное количество элементов в структуре данных, с которой работает машина
        ...
        [MarshalAs(UnmanagedType.ByValTStr, SizeConst = MAX_PATH)]//методы для работы с неуправляемым(машинным) кодом
    ...
    public struct ModuleEntry32
    {
        ...
        [MarshalAs(UnmanagedType.ByValTStr, SizeConst = 256)]//дополнительно имеем 4 элемента, чтобы не было нехватки места для работы машины
    ...
    internal class ToolHelp32
    {
        ...
        [Flags]//битовый флаг для битовых операций с нашим перечислимым типом
        ...
        public IEnumerable<ProcessEntry32> GetProcessList()
        {
            ...
            try
            {
                ...
                handleToSnapshot = CreateToolhelp32Snapshot((uint)SnapshotFlags.All, 0); /*процессы поддающиеся обработке
                (если любой из: куча, процесс, поток, модуль, наследник существуют и мы можем его взять)*/
                ...
                if (Process32First(handleToSnapshot, ref procEntry)) //если есть хотя бы один процесс
                {
                    ...
                    } while (Process32Next(handleToSnapshot, ref procEntry)); //пока процессы не закончатся
                }
            ...
            {
                CloseHandle(handleToSnapshot); //уничтожаем эти процессы
            }
        }

        /// <summary>
        /// Получение перечисления всех потоков определённого процесса
        /// </summary>
        /// <returns></returns>
        /// 
        ...
        public IEnumerable<ThreadEntry32> GetThreadList(uint th32ProcessID)
        {
            ...
            try
            {
                ThreadEntry32 thEntry = new ThreadEntry32(); //создаем экземпляр trEntry
                thEntry.dwSize = (UInt32)Marshal.SizeOf(typeof(ThreadEntry32));//выделяем для него память
                handleToSnapshot = CreateToolhelp32Snapshot((uint)SnapshotFlags.All, th32ProcessID);/*процессы поддающиеся обработке
                (если любой из: куча, процесс, поток, модуль, наследник существуют и мы можем его взять)*/

                if (Thread32First(handleToSnapshot, ref thEntry)) //если есть хотя бы один поток
                {
                    do {
                        if (thEntry.th32OwnerProcessID == th32ProcessID) //если поток относится к нужному процессу
                            yield return thEntry; //возвращаем поток
                    }
                    while (Thread32Next(handleToSnapshot, ref thEntry)); //пока потоки не закончатся
                }
                else
                {
                    yield break;
            ...
            finally { CloseHandle(handleToSnapshot); } //уничтожаем снимок
        }

        /// <summary>
        /// Нахождение максимального приоритета из всех потоков определённого процесса
        /// </summary>

        public int CountMaxPriorityByProcess(uint processId)
        {
            int max = 0;//изначально приоритет = 0
            {
                foreach (ThreadEntry32 thEntry in GetThreadList(processId))//проходимся по всем потокам процесса
                {
                    int curr_prior = thEntry.tpBasePri;  //рассматриваем приоритет текущего потока
                    if (curr_prior > max) //если он больше максимального, то он максимальный
                        max = curr_prior;
                }
            }
            return max; //возвращаем максимальный приоритет всех потоков процесса
        }
        ...
        /// <summary>
        /// Получение перечисления всех процессов, имеющих потоки с большим реальным приоритетом, чем сами процессы
        /// </summary>
        /// <returns></returns>
        /// 


        public IEnumerable<ProcessEntry32> GetProccessesMaxPriorInRange()
        {
            foreach (ProcessEntry32 pe in GetProcessList()) //проходимся по всем процессам
                if (CountMaxPriorityByProcess(pe.th32ProcessID) > pe.pcPriClassBase) //если выполнено условие задачи
                    yield return pe; //добавляем процесс в перечисление
        }
    }
}
