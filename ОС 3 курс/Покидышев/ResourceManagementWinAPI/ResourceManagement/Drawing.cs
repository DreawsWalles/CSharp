...
namespace ResourceManagement
{
    ...
    internal class WorkerDraw : Worker, IDrawingComponent
    {
        ...
        public void Draw(Graphics gr)
        {
            ...
            gr.DrawString(Attempts.ToString(), new Font("Arial", 16), new SolidBrush(Color.Black), X + 5, Y + 3);
        }

        public WorkerDraw(Resource resource, int attempts, string name, int x, int y, uint threadID) : base(resource, attempts, threadID, name)
        {
            X = x;
            Y = y;
            Color = Color.Gray;
