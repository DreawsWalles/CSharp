...
namespace OS.Task3
{
    ...
    public struct ProcessEntry32
    {
        ...
        public uint cntUsage;// хранит значение счетчика ссылок процесса. 
                             //Когда это значение станет равным 0Б операционная система выгрузит процесс
        public uint th32ProcessID; // идентификационный номер процесса
        public IntPtr th32DefaultHeapID; //предназначен для хранения иентификатора для кучи процесса, 
                                        //действующего по умодчанию 
        public uint th32ModuleID; // идентифицирует модуль связанный с процессом 
        public uint cntThreads; // сколько потоков начало выполнятся в слудующем процессе
        public uint th32ParentProcessID; // идентификатор родительского процесса для данного процесса
        public int pcPriClassBase; // базовый приоритет процесса 
        public uint dwFlags; // зарезервировано
        [MarshalAs(UnmanagedType.ByValTStr, SizeConst = MAX_PATH)] 
        public string szExeFile;// содержится строка с ограничивающим нуль-символом, которая представляет собой путь и имя файла EXE-программы
                                //или драйвера, связанного с данным процессом    
