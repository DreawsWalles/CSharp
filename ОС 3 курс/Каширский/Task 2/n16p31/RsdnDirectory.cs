...
namespace n16p31 {
    class RsdnDirectory {
        ...
        /// </summary>
        private static string MakePath (string path) { return Path.Combine(path, "*"); }
        ...
        private static IEnumerable<string> GetInternal (string path, bool isGetDirs) {
            ...
            // Этот дескриптор требуется передавать функции FindNextFile для получения следующих файлов.
        ...
        } //private static IEnumerable<string> GetInternal
        ...
        public static IEnumerable<string> GetFilse (string path) { return GetInternal (path, false); }
        ...
        /// </param>
        /// <returns>Список файлов каталога.</returns>
        public static IEnumerable<string> GetDirectories (string path) { return GetInternal(path, true); }

        /// <summary>
        /// Функция возвращает список относительных путей ко всем подкаталогам
        /// (в том числе и вложенным) заданного пути.
        /// </summary>
        /// <param name="path">Путь для которого нужно получить подкаталоги.</param>
        ...
        public static IEnumerable<string> GetAllDirectories (string path) {
            ...
            foreach (string subDir in GetDirectories(path)) {
                ...
                // Создаем рекурсивно итератор для каждого подкаталога и возвращаем каждый его элемент 
                // в качестве элементов текущего итератора. Этот прием позволяет обойти ограничение 
                // итераторов C# 2.0, связанное с невозможностью вызовов "yield return" из функций, вызываемых
                // из функции итератора. К сожалению, это приводит к созданию временного вложенного итератора
                // на каждом шаге рекурсии, но затраты на создание такого объекта относительно невелики,
                // а удобство очень даже ощутимо
                foreach (string subDir2 in GetAllDirectories(relativePath))
                    yield return subDir2;
            } //foreach string subDir
